...
//		Locale locale = RequestContextUtils.getLocale(request);
		if (principal != null) {
			...
			// dashboard =
			// dashboardService.findOrCreateDashboardByUser(loggedUser);
			dashboard = (Dashboard) request.getSession().getAttribute(SESSION_KEY_DASHBOARD);
			logger.debug("Dasboard loaded from session: "+dashboard);
			if(dashboard == null){
				
				dashboard = this.dashboardWidgets.getDashboard(principal.getName());
				request.getSession().setAttribute(SESSION_KEY_DASHBOARD,dashboard);
			}
...
//		User loggedUser = null;
		if (principal != null) {
			...
			Map<String, List<Widget>> allWidgets = (Map<String, List<Widget>>) request.getSession().getAttribute(SESSION_KEY_WIDGET_LIST);
			...
			if(allWidgets == null ){
				...
				request.getSession().setAttribute(SESSION_KEY_WIDGET_LIST,allWidgets);
			}
...
//					Locale locale = RequestContextUtils.getLocale(request);
//					List queryResults = widgetService.executeQuery(widget,
//							locale);
//					result = toGoogleChartJSON(queryResults);
				Locale locale = RequestContextUtils.getLocale(request);
				List<?> queryResults = this.dashboardWidgets.executeQueryForWidget(principal.getName(), widgetId,locale);
				result = toGoogleChartJSON(queryResults);
...
//			}
			this.dashboardWidgets.updateWidgetPosition(principal.getName(), widgetId, oldPosition, newPosition);
		}
	}
...
//			}
			this.dashboardWidgets.hideShowWidget(widgetId, principal.getName(), DashboardWidgetFacade.HIDE);
...
//				widgetService.saveWidget(widget);
//			}
			this.dashboardWidgets.hideShowWidget(widgetId, principal.getName(), DashboardWidgetFacade.VISIBLE);
			this.clearSession(request, SESSION_KEY_WIDGET_LIST);
...
//	 *            the new position
//	 */
//	private void moveWidgetFromTo(List<Widget> widgets, int oldPosition,
//			int newPosition) {
